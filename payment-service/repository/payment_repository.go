...
func (pr *PaymentRepository) UpdatePayment(payment models.Payment) error {
	_, err := pr.DB.Exec("UPDATE payments SET user_id = $1, order_id = $2, amount = $3, payment_status = $4 WHERE id = $5", payment.UserID, payment.OrderID, payment.Amount, payment.PaymentStatus, payment.ID)
	if err != nil {
		return err
	}
	return nil
